...
// Classe usada para comparar dois objetos do tipo Board com base na heurística
class BoardComparator implements Comparator<Board>{
    public int compare(Board board1, Board board2){
        return board1.heuristic - board2.heuristic;
    }
}

class Greedy{

  // instância do comparador criado acima para ser usada na criação das filas de prioridade
  Comparator<Board> comparator = new BoardComparator();
  // fila de prioridade principal que contém os estados do jogo a serem explorados
  PriorityQueue<Board> queue = new PriorityQueue<>(comparator);
  // fila de prioridade auxiliar usada na geração dos filhos de um estado
  PriorityQueue<Board> temp = new PriorityQueue<>(comparator);
  // estado final esperado
  Board finalBoard;

  // opção de heurística escolhida pelo usuário (1 para misplaced tiles, 2 para manhattan distance)
  int option;
  ...
  // construtor da classe que inicia a busca
  Greedy(Board board, Board finalBoard, int option){
      ...
      Board solution = greedy_visit(finalBoard);
  ...
  void misplacedPieces(Board board){
    ...
    int finalArray[] = this.finalBoard.convertBoardToArray();
    for(int i = 0; i < 16 ; i++){
      if(array[i] != finalArray[i] && array[i] != 0 ){
        counter++;
      }
    }
  ...
  void manhattanDistance(Board board){
    ...
    for(int i = 0; i < 16; i++){
      if(initialPositionArray[i] != finalPositionArray[i] && initialPositionArray[i] != 0){
  ...
  // Função que seleciona a heurística a ser utilizada pelo algoritmo Greedy
  void heuristicagreedy(Board board){
    if(option == 1)
